				...
				switch($info['Type']){
					...
					case 9:
						...
						if($elementProperties[94]){
							$res=dbDoQuery('SELECT `'.$elementProperties[92].'`, `'.implode('`, `',$matchFields[0]).'` FROM `'.$elementProperties[90].'` ' . ($elementProperties[99] ? ' WHERE Lang =  ' . $this->lang->GetId() : '') .($elementProperties[94]?' ORDER BY '.$elementProperties[94].' '.$elementProperties[95]:''),__FILE__,__LINE__);
						}else{
							$res=dbDoQuery('SELECT `'.$elementProperties[92].'`, `'.implode('`, `',$matchFields[0]).'` FROM `'.$elementProperties[90].'`' . ($elementProperties[99] ? ' WHERE Lang =  ' . $this->lang->GetId() : ''),__FILE__,__LINE__);
						}
						while($rec=dbGetRecord($res)){
							if(!$defaultTitle&&$elementProperties[1]){
	...
	function AddRecord(){
		...
		if(is_array($ids)){
			foreach($ids as $id){
				...
				if(!$errorInner){
					dbDoQuery('INSERT INTO `'.$this->document['Table'].'` SET '.$sqlDescription . ($this->document['HasLang'] ? ', Lang = '. $this->lang->GetId() . ' ' : ''),__FILE__,__LINE__);
			...
			foreach($fields as $name=>$info){
				...
				switch($info['Type']){
					...
					case 9:
						...
						preg_match_all('/([A-z0-9]+)/',$elementProperties[91],$matchFields);
						if($elementProperties[94]){
							$resOpt=dbDoQuery('SELECT `'.$elementProperties[92].'`, `'.implode('`, `',$matchFields[0]).'` FROM `'.$elementProperties[90].'`'.$sqlCondition . ($elementProperties[99] ? ($sqlCondition?' AND':' WHERE').' Lang = '.$this->lang->GetId() : '') . ' '.($elementProperties[94]?'ORDER BY '.$elementProperties[94].' '.$elementProperties[95]:''),__FILE__,__LINE__);
						}else{
							$resOpt=dbDoQuery('SELECT `'.$elementProperties[92].'`, `'.implode('`, `',$matchFields[0]).'` FROM `'.$elementProperties[90].'`'.$sqlCondition . ($elementProperties[99] ? ($sqlCondition?' AND':' WHERE').' Lang = '.$this->lang->GetId() : ''),__FILE__,__LINE__);
						}
						$counter=0;
						while($opt=dbGetRecord($resOpt)){
